...
if A==8
    set(handles.edit1,'String',...
       {'PIV Method:' 'This section controls the PIV evaluation technique used to process images.  The listbox indicates the passes that will be used to process each image pair.' ''...
        ...
        'Multiframe (Persoons):' 'Correlates each image pair using a Multigrid DWO. However, in regions with low displacements the alorithm may "step out" to obtain a larger time step / displacement (ie - correlating im1 and im4 instead of im2 and im3). The algorithm described in [4] is used to select the optimal framestep. This method is useful for experiments with large velocity gradients, such as a rising bubble or a jet in quiescent fluid. It has not been designed to work with double-pulsed experiments (where 1/Laser Pulse Sep ~= Camera Frame Rate).' '' ...
        'References:' ...
