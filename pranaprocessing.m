...
%                             imwrite(uint8(im2d),[pltdirec char(wbase(e,:)) sprintf(['%0.' Data.imzeros 'ib.png' ],I1(q))]);
            ...
            fprintf('----------------------------------------------------\n')
            for e=1:P
                fprintf('correlation...                   %0.2i:%0.2i.%02.0f\n',floor(sum(corrtime(e,:))/60),floor(rem(sum(corrtime(e,:)),60)),(rem(sum(corrtime(e,:)),60)-floor(rem(sum(corrtime(e,:)),60)))*100)
                if Valswitch(e)
                    fprintf('validation...                    %0.2i:%0.2i.%02.0f\n',floor(sum(valtime(e,:))/60),floor(rem(sum(valtime(e,:)),60)),(rem(sum(valtime(e,:)),60)-floor(rem(sum(valtime(e,:)),60)))*100)
                end
                if strcmpi(M,'Deform') && mindefloop(e) ~= 1
                    fprintf('velocity interpolation...        %0.2i:%0.2i.%02.0f\n',floor(sum(interptime(e,:))/60),floor(rem(sum(interptime(e,:)),60)),(rem(sum(interptime(e,:)),60)-floor(rem(sum(interptime(e,:)),60)))*100)
                    fprintf('image deformation...             %0.2i:%0.2i.%02.0f\n',floor(sum(deformtime(e,:))/60),floor(rem(sum(deformtime(e,:)),60)),(rem(sum(deformtime(e,:)),60)-floor(rem(sum(deformtime(e,:)),60)))*100)
                end
                if Writeswitch(e)
                    fprintf('save time...                     %0.2i:%0.2i.%02.0f\n',floor(savetime(e)/60),floor(rem(savetime(e),60)),(rem(savetime(e),60)-floor(rem(savetime(e),60)))*100)
                end
            ...
            fprintf('total frame time...              %0.2i:%0.2i.%02.0f\n',floor(eltime/60),floor(rem(eltime,60)),(rem(eltime,60)-floor(rem(eltime,60)))*100)
            frametime(q)=eltime;
...
%                      [Xc,Yc,CC]=PIVensemble(im1,im2,Corr(e),Wsize(e,:),Wres(e, :, :),0,D(e),Zeromean(e),X(Eval>=0),Y(Eval>=0),Ub(Eval>=0),Vb(Eval>=0));
                        ...
                        corrtime=toc(t1);
                        if strcmpi(M,'EDeform') && (e~=1 || defloop~=1)
                            fprintf('deformation %4.0f of %4.0f...      %0.2i:%0.2i.%02.0f\n',q,length(I1dist),floor(deformtime/60),floor(rem(deformtime,60)),(rem(deformtime,60)-floor(rem(deformtime,60)))*100)
                        end
%                         fprintf('correlation %4.0f of %4.0f...      %0.2i:%0.2i.%0.0f Ensemble %%change %0.2e\n',q,length(I1dist),floor(corrtime/60),floor(rem(corrtime,60)),rem(corrtime,60)-floor(rem(corrtime,60)),cnvg_est)
                        fprintf('correlation %4.0f of %4.0f...      %0.2i:%0.2i.%02.0f\n',q,length(I1dist),floor(corrtime/60),floor(rem(corrtime,60)),(rem(corrtime,60)-floor(rem(corrtime,60)))*100)
                    end
                end
...
%                   [Xc,Yc,CC]=PIVensemble(im1,im2,Corr(e),Wsize(e,:),Wres(e, :, :),0,D(e),Zeromean(e),X(Eval>=0),Y(Eval>=0),Ub(Eval>=0),Vb(Eval>=0));
                    ...
                    corrtime=toc(t1);
                    if strcmpi(M,'EDeform') && (e~=1 || defloop~=1)
                        fprintf('deformation %4.0f of %4.0f...      %0.2i:%0.2i.%02.0f\n',q,length(I1),floor(deformtime/60),floor(rem(deformtime,60)),(rem(deformtime,60)-floor(rem(deformtime,60)))*100)
                    end
%                     fprintf('correlation %4.0f of %4.0f...      %0.2i:%0.2i.%0.0f Ensemble %%change %0.2e\n',q,length(I1),floor(corrtime/60),floor(rem(corrtime,60)),rem(corrtime,60)-floor(rem(corrtime,60)),cnvg_est)
                    fprintf('correlation %4.0f of %4.0f...      %0.2i:%0.2i.%02.0f\n',q,length(I1),floor(corrtime/60),floor(rem(corrtime,60)),(rem(corrtime,60)-floor(rem(corrtime,60)))*100)
                end
            end
            ...
            if ~strcmpi(Corr{e},'SPC')
                ...
                peaktime=toc(t1);
                fprintf('peak fitting...                  %0.2i:%0.2i.%02.0f\n',floor(peaktime/60),floor(rem(peaktime,60)),(rem(peaktime,60)-floor(rem(peaktime,60)))*100)
            ...
            if Valswitch(e)
                ...
                valtime=toc(t1);
                fprintf('validation...                    %0.2i:%0.2i.%02.0f\n',floor(valtime/60),floor(rem(valtime,60)),(rem(valtime,60)-floor(rem(valtime,60)))*100)
            ...
            if Writeswitch(e) && defloop == 1
                ...
                savetime=toc(t1);
                fprintf('save time...                     %0.2i:%0.2i.%02.0f\n',floor(savetime/60),floor(rem(savetime,60)),(rem(savetime,60)-floor(rem(savetime,60)))*100)
            end
            U=Uval; V=Vval;
            ...
            if e~=P || defloop ~= 1 %Not the last pass or not finished converging the final pass
                ...
                interptime=toc(t1);
                fprintf('velocity interpolation...        %0.2i:%0.2i.%02.0f\n',floor(interptime/60),floor(rem(interptime,60)),(rem(interptime,60)-floor(rem(interptime,60)))*100)
            end
            
            if defloop == 1
                eltime=toc(tf);
                %output text
                fprintf('total pass time...               %0.2i:%0.2i.%02.0f\n',floor(eltime/60),floor(rem(eltime,60)),(rem(eltime,60)-floor(rem(eltime,60)))*100)
                frametime(e)=eltime;
...
%             fprintf(1, 'Frame completed at %s \n', datestr(now)); % Print the date and time at which frame was completed
            fprintf('----------------------------------------------------\n')
            for e=1:P
                fprintf('correlation...                   %0.2i:%0.2i.%02.0f\n',floor(corrtime(e)/60),floor(rem(corrtime(e),60)),(rem(corrtime(e),60)-floor(rem(corrtime(e),60)))*100)
                if Valswitch(e)
                    fprintf('validation...                    %0.2i:%0.2i.%02.0f\n',floor(valtime(e)/60),floor(rem(valtime(e),60)),(rem(valtime(e),60)-floor(rem(valtime(e),60)))*100)
                end
                if Writeswitch(e)
                    fprintf('save time...                     %0.2i:%0.2i.%02.0f\n',floor(savetime(e)/60),floor(rem(savetime(e),60)),(rem(savetime(e),60)-floor(rem(savetime(e),60)))*100)
                end
                if e~=P
                    fprintf('velocity interpolation...        %0.2i:%0.2i.%02.0f\n',floor(interptime(e)/60),floor(rem(interptime(e),60)),(rem(interptime(e),60)-floor(rem(interptime(e),60)))*100)
                end
            end
            fprintf('total frame time...              %0.2i:%0.2i.%02.0f\n',floor(eltime/60),floor(rem(eltime,60)),(rem(eltime,60)-floor(rem(eltime,60)))*100)
